...
@interface CameraToolbar () // private interface, basically, while .h file is public interface
...
- (void) createConstraints {
    ...
    NSArray *purpleViewHorizontalConstraints = [NSLayoutConstraint constraintsWithVisualFormat:@"[_leftButton][_purpleView][_rightButton]" options:NSLayoutFormatAlignAllBottom metrics:nil views:viewDictionary];  // don't need an H here?
    NSArray *purpleViewVerticalConstraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|[_purpleView]" options:kNilOptions metrics:nil views:viewDictionary];  // don't need a botton | here?
    
    NSArray *allConstraintArrays = @[allButtonsHorizontalConstraints, leftButtonVerticalConstraints, cameraButtonVerticalConstraints, rightButtonVerticalConstraints, whiteViewHorizontalConstraints, whiteViewVerticalConstraints, purpleViewHorizontalConstraints, purpleViewVerticalConstraints]; // order, so the purple and white views must be partly transparent?  they are not though..  so how can we see the buttons below them? maybe they are first in layoutSubviews.
...
- (void) layoutSubviews {
    [super layoutSubviews];  // uses constraints we already set, plus the extra rounding of purple view's corners below
