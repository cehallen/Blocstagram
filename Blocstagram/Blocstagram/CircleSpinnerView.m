...
// notice all these setters of properties we made can be overridden.  just make sure to use the IVAR to manually set them with the params.  does setFrame above also get a setter method from implicity non-property but kind of property of the frame?  yes.. think of it this way:  you can call xyz.frame.  therefore it's an attribute, which has automatic getters, and here you see it has automatic setter functions too, which you can override!  good to know.
...
// can't tell if this is auto-called on instantiation or we call it manually (should be autocalled, since it's an UIView API method.  No, it's manually called in LikeButton.m.  
...
// where and how is this called?  an autocall method to fit "the receiver's subviews"?  what is the receiver.. CircleSpinner?
