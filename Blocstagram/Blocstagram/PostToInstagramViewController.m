...
//  Blocstagram
//
//  Created by Christopher Allen on 10/27/15.
...
@interface PostToInstagramViewController () <UICollectionViewDataSource, UICollectionViewDelegate, UIDocumentInteractionControllerDelegate, FilterViewCellDelegate>
...
- (void) addCIImageToCollectionView:(CIImage *)CIImage withFilterTitle:(NSString *)filterTitle {
    ...
    if (image) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{  // "on the main thread, adds completed UIImage and filter title to the arrays, and tells the collection view that a new item is available."
            // SO, this proves we weren't on the main thread for a certain period of time before this code.  where is the non-main thread called on before this?  wherever this method is called?
