...
// note, it's also written in C not obj-c.
// check here for good intro to affine transformations https://en.wikipedia.org/wiki/Transformation_matrix#Affine_transformations especially the image to the right showing what 'translate' and 'scale' mean
- (UIImage *) imageWithFixedOrientation {
    ...
    switch (self.imageOrientation) {
        ...
        case UIImageOrientationDownMirrored:
            ...
            transform = CGAffineTransformRotate(transform, M_PI);  // rotates 180º
            break;
            
        case UIImageOrientationLeft:
        case UIImageOrientationLeftMirrored:
            transform = CGAffineTransformTranslate(transform, self.size.width, 0);
            transform = CGAffineTransformRotate(transform, M_PI_2); // rotates 90º clockwise
            break;
...
// since aspect ratio of device screen is not same as aspect ratio of device's camera.  I think it basically blows up the smaller edge to match the bigger one, and the rest overlaps to be cropped later.  like UIImage "Aspect Fill"
